...
## The purpose of this script is to perform EM to further break the components from HDP to signatures

# expectation maximisation to divvy up hdp signatures into pcawg signatures.

library("lattice")
hist.cols <- rep(c("#16bdebff", "#000000ff", "#e22926ff", "#999999ff", "#9fce67ff", "#ecc6c5ff"), each=16)
output_dir="/Users/so11/phd/so11_lustre/somatic_ibd_p1/signature_extraction/hdp/snvs/EM_reconstruction/"

# i) For known signatures (e.g. when HDP calls something as “SBS 1”)
#   a.      we calculate the cosine similarity between the HDP version of the signature, and 
#           the PCAWG version of the signature.
#   b.      If the cosine similarity is below a certain threshold (0.95 seems to work well), 
#           we then apply an expectation maximisation (EM) algorithm to deconvolute this 
#           signature into its constituents. E.g. the preconditioned HDP SBS1 goes to 
#           38.5% SBS1, 47.5% SBS5, and 15.0% SBS18. Reassuringly, 38% of the mutations 
#           that make up the HDP SBS1 are C to T at CpG, so that fits well.
...
#   b.      We then reconstitute the signature by adding up the PCAWG signatures in the proportion 
#           that EM gives us (e.g. IDN1 is 27% ID1, 14% ID2, 20% ID5 etc). I have tried only using 
#           signatures that contribute >10% of the mutations to avoid overfitting.
#   c.      We compute the cosine similarity of the reconstituted signature to the known signature. 
#           If that is less than a certain threshold, then we consider that the signature is truly novel. 
#           Otherwise, we break it down into its constituents and present the data in that way.

# read in the signature extraction results.
hsbs <- data.frame(t(comp_categ_distn(luad_multi)$mean))
## Hard coded - change if you re-run the signature extraction
colnames(hsbs) <- c("Unassigned", "SBS1", "SBS5", "N1", "SBS18", "SBS32", "N2", "N3",
                    "SBS17b", "N4", "N5", "N6", "N7","SBS13","SBS2","SBS17a")

# read in the pcawg sigs
psbs <- read.csv("/Users/so11/phd/so11_nfs/somatic_ibd_p1/resources/pcawk_signatures/sigProfiler_SBS_signatures_summing_to_one.csv", header = T, row.names = 1, stringsAsFactors = F)
...
s <- cosmat
write.table(cosmat, paste(output_dir, "Cosine_similarities.txt", sep=""), sep="\t", col.names = T, row.names = T, quote=F)

pdf(paste(output_dir, "Cosine_similarities.pdf",sep=""), height=5, width=15)
...
# SBS1, 5 and 18 need to be split.
...
sample_list = c("Unassigned", "SBS1","SBS5","SBS18")
...
pdf(paste(output_dir, "EM_break_up_known_sigs.pdf", sep=""), height=5, width=10)
color.palette = colorRampPalette(c("white", "orange", "purple"))
levelplot((s[dim(s)[1]:1,]),col.regions=color.palette, aspect="fill", scales=list(x=list(rot=90)))
dev.off()

write.table(s, paste(output_dir, "hdp_known_sigs_broken_down_into_pcawg_gd_sigs.txt", sep=""), sep="\t", col.names=T, row.names = T, quote=F)



## We can see that there is no point trying to split SBS18 as no other signature contributes more than 10% of the
## mutations for that component. For SBS5, only SBS40 contributes >10% but these are so similar that I think I'm
## better off not breaking SBS5 up. 

constit <- rownames(signature_fraction[signature_fraction[,c("SBS1")]>0.1,])
...
pdf(paste(output_dir, "EM_break_up_SBS1_into_pcawg_1-5-18.pdf", sep=""), height=5, width=10)
color.palette = colorRampPalette(c("white", "orange", "purple"))
levelplot((s[dim(s)[1]:1,]),col.regions=color.palette, aspect="fill", scales=list(x=list(rot=90)))
dev.off()

write.table(s, paste(output_dir, "hdp_SBS_broken_down_into_pcawg_1-5-18.txt", sep=""), sep="\t", col.names=T, row.names = T, quote=F)
s
# HDP SBS 1 is: 38.5% psbs1, 47.5% psbs5, 14.0% psbs18.

# reconstitute HDP SBS1 using those PCAWG sigs. Calculate the cosine similarity 
# between the original and the reconstituted pcawg sig.
constit
reconsbs <- rep(0, 96)
for (ct in constit) {
...
pdf(paste(output_dir, "HDP_SBS1_reconstitution.pdf", sep=""))
par(mfrow=c(5,1))
par(mar=c(1,2,4,1))
...
# only split it into the good signatures.
gdsigs <- c("SBS1", "SBS2", "SBS3", "SBS5", "SBS13", "SBS16", "SBS17a", "SBS17b", "SBS18", "SBS25", "SBS28", "SBS30","SBS32","SBS35", "SBS37", "SBS40", "SBS41", "SBS43", "SBS45", "SBS49") 
sigs <- as.matrix(psbs[,gdsigs])
signatures <- t(sigs)
...
pdf(paste(output_dir, "EM_break_up_novel_sigs.pdf", sep=""), height=5, width=10)
color.palette = colorRampPalette(c("white", "orange", "purple"))
levelplot((s[dim(s)[1]:1,]),col.regions=color.palette, aspect="fill", scales=list(x=list(rot=90)))
dev.off()

write.table(s, paste(output_dir, "hdp_novel_sigs_broken_down_into_pcawg_gd_sigs.txt", sep=""), sep="\t", col.names=T, row.names = T, quote=F)


pdf(paste(output_dir,"Breaking_down_novel_sigs_into_pcawg_sigs_by_nmf.pdf", sep=""))
